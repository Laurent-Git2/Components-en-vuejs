...
- Para cualquier tipo de *duda o consulta* se pueden abrir ~Issues~ haciendo referencia a la profesora mediante el texto ~@elisanoguera~ dentro del texto del ~Issue~.
- Una vez *finalizada* la tarea se debe crear un ~Issue~ en el repositorio haciendo referencia a la profesora incluyendo el texto ~@elisanoguera~ dentro del ~Issue~.
